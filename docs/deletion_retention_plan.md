...
0a. **New, from Rev 4**: `erasure_requests`' own retention treatment is unresolved — flagged in §4 as likely needing a *longer* retention window than everything else in this plan (it's compliance evidence, not disposable log noise), which cuts against the short-window instinct used everywhere else here. Needs an explicit decision, not just a TBD number.
0b. **New, from Rev 4**: the internal Rinsion platform dashboard has no existing "erasure requests" queue UI (mirroring the Manual Payments Queue pattern for `pending_payments`) — out of scope to design here since the instruction was RPC/wrapper-level only, but noted as the natural next surface once this is built.
